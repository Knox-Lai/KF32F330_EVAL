...
# 16 "E:\\KF_Workspace\\Project_WorkSpace\\KF32F330_EVAL\\BSP/bsp.h" 2
...
# 17 "E:\\KF_Workspace\\Project_WorkSpace\\KF32F330_EVAL\\BSP/bsp.h" 2
...
# 22 "E:\\KF_Workspace\\Project_WorkSpace\\KF32F330_EVAL\\BSP/bsp.h" 2
...
# 6 "../Driver/ota_app.c" 2
# 1 "E:\\KF_Workspace\\Project_WorkSpace\\KF32F330_EVAL/utilities.h" 1
...
void ota_app_start(void)
{
    fprintf(((STREAM *)0x40000980UL), "ota_app_start\r\n");


    ota.fileChecksum = 0;
    ota.fileBytesNum = 0;
    ota.bytesCount = 0;
    ota.lastPktIndex = 800;
...
void ota_app_set_info(uint8_t* data)
{
    ...
    temp = uint8_to_uint32(data, 4);
    if (temp > ((0x0400UL) * 98UL))
    {
        ERRLIST_add(22);
        return;
    }
...
void ota_app_recvpkg(uint8_t* data)
{
    ...
    if (ota.firmwareIdFlag == 0)
    {
        fprintf(((STREAM *)0x40000980UL), "firmwareId error!!!\r\n");
        return;
    }


    packageIndex = data[0] | data[1] << 8;
    if (packageIndex > 800)
    {
        ERRLIST_add(20);
        return;
    ...
    if (packageIndex == ota.lastPktIndex)
    {
        return;
    }

    ota.lastPktIndex = packageIndex;
    data += 2;


    if ((ota.bytesCount + 128) <= ota.fileBytesNum)
    {
        ota.bytesCount += 128;
        ota.fileChecksum += CalChecksum(data, 128);
    }
    else
    {
    ...
    addr = (((0x00000000UL) + ((0x0400UL) * 12UL)) + ((0x0400UL) * 98UL)) + 128 * packageIndex;

    if ((addr & (((0x0400UL) * 1UL) - 1)) == 0)
    {
        flash_erase_page(addr);
    }

    flash_write_buf(addr, data, 128);

    fprintf(((STREAM *)0x40000980UL), "write data %d\r\n", ota.lastPktIndex);
...
# 142 "../Driver/ota_app.c"
    ...
    md5_flash((((0x00000000UL) + ((0x0400UL) * 12UL)) + ((0x0400UL) * 98UL)), ota.fileBytesNum, md5Result);
    memcpy(&flgData[index], &md5Result, sizeof(md5Result));
    index += 16;

    flash_write_buf((((((((0x00000000UL) + ((0x0400UL) * 12UL)) + ((0x0400UL) * 98UL)) + ((0x0400UL) * 98UL)) + ((0x0400UL) * 1UL) * 42) + ((0x0400UL) * 1UL) * 2) + ((0x0400UL) * 1UL) * 2), flgData, sizeof(flgData));


    fprintf(((STREAM *)0x40000980UL), "md5:");
    for (int i = 0; i < 16; i++)
    {
        fprintf(((STREAM *)0x40000980UL), "%02x", flgData[index + i]);
    }
    fprintf(((STREAM *)0x40000980UL), "\r\n");
    fprintf(((STREAM *)0x40000980UL), "ota_app_end\r\n");
